...
function updateFixed () { 
    $(".js-fixed").each(function(i) { 
        $(this).css({"z-index": 98 - i,"position":"static"});
        var $par= $(this).parents(".fixed-wrap"); 
            var h1 = $(this).height( ) - $(this).find(".t-head-fixed").outerHeight(true);
            $(this).css({
                "left": $(this).position().left,
                "right": $(window).width() - $(this).position().left - $(this).outerWidth()
            });
            $(this).attr("fixed-direction") === "bottom" ?
                $(this).css("bottom", "0") :
                $(this).css({
            ...
            $(this).addClass("fixed").css({"position":""}); 
        $par.css("height",h1)
    });
}
...
function tablecolfixedInit() {
 ...
 !(function(w, $) {    
    ...
    var  mcombobox=function(dom,opt ){
        this.dom=dom;
        this.opt=opt;
        this.init();
        this.handle();
    ...
    mcombobox.prototype={
        init:function(){ 
            var _this=this;
            $(_this.dom).find(".input-view").addClass(" form-control dropdown-toggle")
        },
        
        getValue:function(){ 
            var _this=this;
            var $re=$(_this.dom).find(".input-result")  , $view=$(_this.dom).find(".input-view") ;
            if($view.val()!=$re.attr("data-text")){
                return null;
            }
            else return $re.val();
        },
        getText:function(){
            var _this=this;
                return $(_this.dom).find(".input-view").val();
        },
        setData:function(json){
            var html='';
            for(var i in json){
                  html="<li value='"+i+"''>"+json[i]+"</li>";
            }
            var drop;
            if($(this.dom).find(".dropdown-menu").length===0){
                drop=$(this.dom).append("<ul class='.dropdown-menu'></ul>");
            }
            else{
                drop=$(this.dom).find(".dropdown-menu");
            }
            drop.append(html);
        } ,
        ...
        handle:function(){
            $(this.dom).find(".dropdown-toggle").on("click keyup change", function() {
                var filter = $(this).val().trim().toLowerCase();
                $drop = $(this).parents(".filter-input").find(".dropdown-menu");
                $lis = $drop.find("li");
                if (filter === "") $lis.show();
                $lis.each(function() {
                    var text = $(this).text();
            ...
            $(this.dom).on("click", ".dropdown-menu li", function() {
                $drop = $(this).parents(".filter-input");
                $drop.find(".input-view").val($(this).text()) ;
                $drop.find(".input-result").val($(this).attr("value")||"").attr("data-text",$(this).text());
            });   
    ...
    $.fn.mcombobox=function(){ 
        if( this.length !== 0  ){

         var  datacombobox=$(this).data("mcombobox" );
           if(! datacombobox)
           {
                datacombobox =  new mcombobox(this, arguments) ;
                 $(this).data("mcombobox" ,datacombobox) ;
                return  datacombobox;
            }
           else{
            return  datacombobox;
